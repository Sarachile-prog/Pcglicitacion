...
export default function CompaniesManagementPage() {
  ...
  const handleCreateCompany = async () => {
    ...
    try {
      ...
      toast({ title: "Empresa Creada", description: "Ya puedes vincular usuarios a este tenant." })
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 max-w-7xl mx-auto pb-20">
        ...
        <Dialog open={isAddingCompany} onOpenChange={setIsAddingCompany}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              ...
              <Button onClick={handleCreateCompany} className="w-full h-12 font-black uppercase italic" disabled={isSyncing}>
        ...
        <CardHeader className="bg-accent/10 border-b p-6">
          <div className="flex justify-between items-center">
            <CardTitle className="text-xl font-black flex items-center gap-2 text-primary uppercase italic tracking-tighter">
              <Ticket className="h-5 w-5 text-accent" /> Prospectos en Espera
            </CardTitle>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-white/50">
              <TableRow>
                <TableHead className="font-black uppercase text-[10px] py-4">Usuario Registrado</TableHead>
                <TableHead className="font-black uppercase text-[10px]">Tipo</TableHead>
                <TableHead className="text-right font-black uppercase text-[10px] pr-6">Acción de Activación</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isUsersLoading ? (
                ...
                <TableRow><TableCell colSpan={3} className="text-center py-10 italic text-muted-foreground font-medium text-xs">No hay nuevos registros pendientes.</TableCell></TableRow>
              ) : prospects.map((u) => (
                <TableRow key={u.id} className="hover:bg-white/80">
                  <TableCell className="py-4 pl-6">
                    <div className="flex flex-col">
                      <span className="font-bold text-sm text-primary">{u.email || "Usuario Anónimo (Demo)"}</span>
                      <span className="text-[9px] font-mono text-muted-foreground uppercase">{u.id}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline" className={cn("text-[8px] font-black uppercase", u.email ? "border-primary text-primary" : "border-orange-400 text-orange-600")}>
                      {u.email ? "EMAIL" : "ANÓNIMO"}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right pr-6">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button size="sm" className="font-black text-[10px] uppercase italic h-8 gap-2">
                          Vincular a Empresa <ChevronRight className="h-3 w-3" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-56">
                        <DropdownMenuLabel className="text-[9px] uppercase font-black">Selecciona Destino</DropdownMenuLabel>
                        <DropdownMenuSeparator />
                        {companies?.map(c => (
                        ...
                        {(!companies || companies.length === 0) && <DropdownMenuItem disabled className="text-[10px] italic">No hay empresas creadas</DropdownMenuItem>}
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
      ...
      <Card className="border-none shadow-xl overflow-hidden rounded-3xl">
        <CardHeader className="border-b bg-muted/20 p-6 flex flex-col md:flex-row justify-between gap-4">
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-muted/50">
              <TableRow>
                <TableHead className="font-black uppercase text-[10px] py-4">Empresa / Usuarios</TableHead>
            ...
            <TableBody>
              {isCompaniesLoading ? (
                ...
                return (
                  <TableRow key={company.id} className="hover:bg-muted/30 align-top">
                    <TableCell className="py-6 pl-6 space-y-4">
                      <div className="space-y-1">
                        <p className="font-black text-primary uppercase italic tracking-tighter text-lg leading-none">{company.name}</p>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="space-y-1">
                          {companyUsers.map(u => (
                            <div key={u.id} className="flex items-center justify-between bg-white/50 p-2 rounded-lg border text-[10px]">
                              <div className="flex flex-col">
